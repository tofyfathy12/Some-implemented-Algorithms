...
int main()
{
    ...
    {
        printf("Current mylistmy1: length = %d\n", mylistmy1->length);
        print_list(mylistmy1);
        delete (mylistmy1, index);
    }
    printf("Final mylistmy1: length = %d\n", mylistmy1->length);
    print_list(mylistmy1);
    ...
    for (int i = 0; i < sublen; i++)
    {
        ...
        printf("Current sublist: length = %d\n", sub->length);
        print_list(sub);
    }
...
LinkedList *reversed(LinkedList *list)
{
    ...
    if (new_list != NULL)
    {
        if (!is_empty(list))
        {
            Node *prev = NULL, *cur = list->head, *next = get_next(cur);
            Node *cur_new = create_node(get_data(cur)), *prev_new = NULL;
            new_list->tail = cur_new;
            while (next != NULL)
            {
                set_next(cur_new, prev_new);
                prev = cur;
                cur = next;
                next = get_next(next);
                prev_new = cur_new;
...
int delete(LinkedList *list, int index)
{
    ...
    else
    {
        ...
        if (index > 0 && index < (list->length - 1))
            set_next(prev_node, get_next(cur_node));
        else if (index == 0)
